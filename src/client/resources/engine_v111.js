...
(function (global, factory) {
  ...
  function getFormattedComponentStack(elm) {
    ...
    do {
      ArrayPush.call(
        componentStack,
        `${indentation}<${StringToLowerCase.call(
          tagNameGetter.call(currentNode)
        )}>`
      );
  ...
  function updateAttrs(oldVnode, vnode) {
    ...
    if (process.env.NODE_ENV !== "production") {
      assert.invariant(
        isUndefined(oldAttrs) ||
          keys(oldAttrs).join(",") === keys(attrs).join(","),
  ...
  class ReactiveProxyHandler {
    ...
    setPrototypeOf(shadowTarget, prototype) {
      if (process.env.NODE_ENV !== "production") {
        throw new Error(
          `Invalid setPrototypeOf invocation for reactive proxy ${toString$1(
            this.originalTarget
    ...
    if (process.env.NODE_ENV !== "production") {
      assert.invariant(
        isUndefined(oldProps) ||
          keys(oldProps).join(",") === keys(props).join(","),
  ...
  */
  ...
  function getNodeRestrictionsDescriptors(node, options) {
    ...
    const originalTextContentDescriptor = getPropertyDescriptor(
      node,
      "textContent"
    );
    const originalNodeValueDescriptor = getPropertyDescriptor(
      node,
      "nodeValue"
    );
    ...
    return {
      appendChild: generateDataDescriptor({
        value(aChild) {
          if (this instanceof Element && isFalse$1(options.isPortal)) {
            logError(
              portalRestrictionErrorMessage("appendChild", "method"),
              this
            );
          }
      ...
      insertBefore: generateDataDescriptor({
        value(newNode, referenceNode) {
          if (
            !isDomMutationAllowed &&
            this instanceof Element &&
            isFalse$1(options.isPortal)
          ) {
            logError(
              portalRestrictionErrorMessage("insertBefore", "method"),
              this
            );
          }
      ...
      removeChild: generateDataDescriptor({
        value(aChild) {
          if (
            ...
            logError(
              portalRestrictionErrorMessage("removeChild", "method"),
              this
            );
          }
      ...
      replaceChild: generateDataDescriptor({
        value(newChild, oldChild) {
          if (this instanceof Element && isFalse$1(options.isPortal)) {
            logError(
              portalRestrictionErrorMessage("replaceChild", "method"),
              this
            );
          }
      ...
      nodeValue: generateAccessorDescriptor({
        ...
        set(value) {
          if (
            ...
            logError(
              portalRestrictionErrorMessage("nodeValue", "property"),
              this
            );
          }
      ...
      textContent: generateAccessorDescriptor({
        ...
        set(value) {
          if (this instanceof Element && isFalse$1(options.isPortal)) {
            logError(
              portalRestrictionErrorMessage("textContent", "property"),
              this
            );
          }
  ...
  function getShadowRootRestrictionsDescriptors(sr, options) {
    ...
    forEach.call(
      getOwnPropertyNames(BlackListedShadowRootMethods),
      (methodName) => {
  ...
  function patchCustomElementWithRestrictions(elm, options) {
    const restrictionsDescriptors = getCustomElementRestrictionsDescriptors(
      elm,
      options
    );
  ...
  function patchLightningElementPrototypeWithRestrictions(proto) {
    defineProperties(
      proto,
      getLightningElementPrototypeRestrictionsDescriptors(proto)
    );
  }
  /*
   * Copyright (c) 2018, salesforce.com, inc.
  ...
  function c(sel, Ctor, data, children) {
    ...
    if (process.env.NODE_ENV !== "production") {
      ...
      assert.isTrue(
        arguments.length === 3 || isArray$1(children),
  ...
  function i(iterable, factory) {
    ...
    if (process.env.NODE_ENV !== "production") {
      assert.isTrue(
        ...
        `Invalid iterator function for "${toString(
          iterable
        )}" in ${vmBeingRendered}.`
      );
    }
    ...
    while (last === false) {
      ...
      if (process.env.NODE_ENV !== "production") {
        ...
        forEach.call(vnodes, (childVnode) => {
          if (
            !isNull(childVnode) &&
            isObject$1(childVnode) &&
            !isUndefined(childVnode.sel)
          ) {
  ...
  function validateFields(vm, html) {
    ...
    const { component } = vm; // validating identifiers used by template that should be provided by the component
  ...
  function getMarkName(phase, vm) {
    return `<${StringToLowerCase.call(
      tagNameGetter.call(vm.elm)
    )} (${vm.idx})> - ${phase}`;
  }
    ...
    return {
      ...
      set(newValue) {
        ...
        if (process.env.NODE_ENV !== "production") {
          ...
          assert.invariant(
            !isObject$1(newValue) || isNull(newValue),
  ...
  function BaseLightningElementConstructor() {
    ...
    if (process.env.NODE_ENV !== "production") {
      assert.isTrue(
        "cmpProps" in vmBeingConstructed,
  ...
  function wireDecorator(target, prop, descriptor) {
    ...
    return createTrackedPropertyDescriptor(
      target,
      prop,
      isObject$1(descriptor) ? descriptor.enumerable === true : true
    );
  }
  /**
  ...
  function api$1(target, propName, descriptor) {
    ...
    if (process.env.NODE_ENV !== "production") {
      ...
      if (isObject$1(descriptor) && isFunction(descriptor.set)) {
        assert.isTrue(
          isObject$1(descriptor) && isFunction(descriptor.get),
  ...
  const { getHiddenField: getHiddenField$4 } = fields;
  ...
  function buildCustomElementConstructor(Ctor, options) {
    ...
    return (
      ...
      (_a.observedAttributes = ArrayMap.call(
        getOwnPropertyNames(props),
